...
class Chroma:
    ...
    def from_chunks(self, chunks: list[Document]) -> list[str]:
        ...
        """
        # 保留原文大小写与非 ASCII（中文等），否则 unidecode 会把中文转写为拼音
        texts = [
            clean(doc.page_content, no_emoji=True, to_ascii=False, lower=False)
            for doc in chunks
        ]
    ...
    def similarity_search_with_threshold(
        ...
        Args:
            ...
            filter : dict, optional
                Chroma `where` filter on metadata, e.g. ``{"document_id": {"$in": [...]}}``.
        ...
        docs_and_scores = self.similarity_search_with_relevance_scores(query=query, k=k, filter=filter)
    ...
    def similarity_search_with_relevance_scores(
        ...
        Args:
            ...
            filter: Chroma `where` filter on metadata.
